...
This wires the contractor share links / QR codes (`https://renovateconnect.com/b/:id`)
...
2. **+ Capability → Associated Domains**. Confirm the entry
   `applinks:renovateconnect.com` (Xcode will point the target's
...
2. Deploy `web/` to the domain `renovateconnect.com`.
3. Verify the file is live and valid JSON:
   `curl -i https://renovateconnect.com/.well-known/apple-app-site-association`
   (200, `content-type: application/json`, your appID in `details[0].appID`).

## 3. Verify end-to-end
- Install the app on a device, then tap a `https://renovateconnect.com/b/<id>`
...
- Apple caches the AASA at install time; if it doesn't work, reinstall the app
  ...
  `swcutil dl -d renovateconnect.com` on macOS to debug.
